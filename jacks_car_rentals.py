...
def transition_prob(state: tuple[int], action: int, next_state: tuple[int]) -> float:
    ...
    for rented in range(min_cars_to_rent, loc_1 + 1):
        ...
        if returned < 0:
            ...
            prob_returned = sf(returned, LOC_1_RETURN_LAMBDA) # TODO
    ...
    for rented in range(min_cars_to_rent, loc_2 + 1):
        ...
        if loc_2_new < MAX_CARS_LOC_2:
            ...
            prob_returned = sf(returned, LOC_2_RETURN_LAMBDA) # TODO
        # Compute the probability of seeing exactly `rented` cars being rented out.
...
def expected_rewards(state: tuple[int], action: int, add_non_linearity: bool = True) -> float:
    ...
    # TODO
    res = 0
    if not add_non_linearity:
        for i in range(MAX_CARS_LOC_1):
            for j in range(MAX_CARS_LOC_2):
                res += transition_prob(state, action, (i, j)) * (RENTAL_REWARD * (expectation(LOC_1_RENTAL_LAMBDA, state[0]) + expectation(LOC_2_RENTAL_LAMBDA, state[1])) - abs(action) * MOVE_COST)
    return res
...
def q(state: np.ndarray, action: int, V: np.ndarray, add_non_linearity: bool) -> float:
    ...
    # TODO
    res = 0
    x = expectation(LOC_1_RENTAL_LAMBDA, state[0] - action) + expectation(LOC_2_RENTAL_LAMBDA, state[1] + action)
    if not add_non_linearity:
        for i in range(MAX_CARS_LOC_1):
            for j in range(MAX_CARS_LOC_2):
                res += transition_prob((state[0], state[1]), action, (i, j)) * (RENTAL_REWARD * (x) - abs(action) * MOVE_COST + GAMMA * V[i][j])
    else:
        for i in range(MAX_CARS_LOC_1):
            for j in range(MAX_CARS_LOC_2):
                res += transition_prob((state[0], state[1]), action, (i, j)) * (RENTAL_REWARD * (x) - max(0, abs(action) - 1) * MOVE_COST - STORAGE_COST * (max(0, i - action - 10) + max(0, j + action - 10))+ GAMMA * V[i][j])
    return res
...
def policy_evaluation(policy: np.ndarray, V: np.ndarray, add_non_linearity: bool, threshold: float):
    ...
    # TODO
    delta = 10000000000
    new_V = [[0 for i in range(21)] for j in range(21)]
    while delta > threshold:
        delta = 0
        for i in range(V.shape[0]):
            for j in range(V.shape[1]):
                new_V[i][j] = q(np.array([i, j]), policy[i][j], V, add_non_linearity)
        ...
        for i in range(V.shape[0]):
            for j in range(V.shape[1]):
                ...
                for a in range(-MAX_CARS_MOVED, MAX_CARS_MOVED + 1):
                    val = q(np.array([i, j]), a, V, add_non_linearity)
                    if val > opt:
                        opt = val
                        act = a
                if policy[i][j] != act:
...
def value_iteration(V: np.ndarray, add_non_linearity: bool, threshold: float) -> np.ndarray:
    ...
    delta = 10000000000
    new_V = [[0 for i in range(21)] for j in range(21)]
    policies = [[0 for i in range(21)] for j in range(21)]
    while delta > threshold:
        delta = 0
        for i in range(V.shape[0]):
            for j in range(V.shape[1]):
